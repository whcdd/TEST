...
//	length_H = (InputBuffer[3])<<8;
	...
	{
		//向上位机返回包头校验错误字
		DXL_SendData(headerr,2);//test
		return;
	}
	if( length != (UART4RxLen-4))
	{
		//向上位机返回长度校验错误字
		DXL_SendData(&UART4RxLen,1);//test
		return;
	}
...
//    CRC_H = CRC16 >> 8;
//		
//	if((InputBuffer[UART4RxLen-2] != CRC_L) || (InputBuffer[UART4RxLen-1] != CRC_L))
//	{
//		//向上位机返回CRC校验错误字
//		return;
//	}	
...
//	USB2UART_SendData(cmdBuffer,sizeof(cmdBuffer)/sizeof(u8));
...
//		DXL_SendData(InputBuffer,cmdLength);
//		InputBuffer2 = (u8 *)malloc(cmdLength*sizeof(u8));
//		memset(InputBuffer2,0,cmdLength);
